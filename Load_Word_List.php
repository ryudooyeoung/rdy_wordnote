...
function Load_Word($day, $date){
	...
	//오늘기준
		...
		if($date!=""){ 
			$query = "SELECT * FROM Word_Eng WHERE date = '{$date}' AND flag = '0' 
			          UNION
					  SELECT * FROM Word_Eng WHERE date = '{$date}' AND flag = '1' "; 
			
		}
		else{
			$today = date("Y-m-d",strtotime("now", $time));
			$query = "SELECT * FROM Word_Eng WHERE date = '{$today}' AND flag = '0' 
					  UNION
					  SELECT * FROM Word_Eng WHERE date = '{$today}' AND flag = '1' "; 
	...
	//이전
		...
		if($date!=""){ 
			...
			$query = "SELECT * FROM Word_Eng WHERE date = '{$result_day}' AND flag = '0'
			          UNION
					  SELECT * FROM Word_Eng WHERE date = '{$result_day}' AND flag = '1' "; 
		}
		else{ 
